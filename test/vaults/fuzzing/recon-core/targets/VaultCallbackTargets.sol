...
abstract contract VaultCallbackTargets is BaseTargetFunctions, Properties {
    ...
    function asyncRequests_fulfillRedeemRequest(uint128 currencyPayout, uint128 tokenPayout, uint128 cancelledShares, uint256 investorEntropy) public notGovFuzzing updateGhostsWithType(OpType.ADMIN) {
        address investor = _getRandomActor(investorEntropy);
    ...
    // function asyncRequests_fulfillCancelDepositRequest(uint128 currencyPayout, uint128 cancelledShares, uint256 investorEntropy) public notGovFuzzing updateGhostsWithType(OpType.ADMIN) {
    //     /// === CLAMP `currencyPayout` === ///
    //     address investor = _getRandomActor(investorEntropy);
